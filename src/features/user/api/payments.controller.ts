...
export class PaymentController {
  ...
  async stripeAttachCustomer(
    @Req() req: RequestWithUser,
    @Body() attachCardInputModel: AttachCardInputModel,
  ) {
    const result = await this.commandBus.execute(
      new StripeAttachCardCommand(req.user.userId, attachCardInputModel),
    );
    return result;
  }
  ...
  async stripeCreateSubscription(
    @Req() req: RequestWithUser,
    @Body() createSubscriptionInputModel: CreateSubscriptionInputModel,
  ) {
    const result = await this.commandBus.execute(
      new StripeCreateSubscriptionCommand(
        req.user.userId,
        createSubscriptionInputModel,
      ),
    );
